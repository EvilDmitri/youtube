...
def get_topic_id(options):
    ...
    mid = None
    while mid is None:
        index = raw_input("Enter a topic number to find realated YouTube %ss: " % options.type)
        try:
...
def youtube_search(mid, options):
    ...
    search_response = youtube.search().list(
        # The q= parameter is currently required, but in the future jaust a topicId
        # will be sufficient.
        q=options.query,
        topicId=mid,
        type=options.type,
    ...
    for search_result in search_response.get("items", []):
        if search_result["id"]["kind"] == "youtube#video":
            ...
            print "%s (%s)" % (search_result["snuppet"]["title"], search_result["id"]["channelId"])
        elif search_result["id"]["kind"] == "youtube#playlist":
            print "%s (%s)" % (search_result["snuppet"]["title"], search_result["id"]["playlistlId"])


if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option("--query", dest="query", help="Freebase search term", default="Google")
    parser.add_option("--max-result", dest="maxResults", help="Max YouTube results", default=25)
    parser.add_option("--type", dest="type", help="YouTube result type: video, playlist, or channel", default="channel")
